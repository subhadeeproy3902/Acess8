...
const UpdateNotes = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const urlInDev = `http://localhost:5000/api/notes/updateNote/${id}`;
    const urlInProd = `https://mynly.vercel.app/api/notes/updateNote/${id}`;
    ...
    setTimeout(() => {
      setIsLoading(false);
      navigate("/dashboard/notes-saver");
    }, 2000);
  };

  useEffect(() => {
    const getNote = async () => {
      const urlInDev = `http://localhost:5000/api/notes/getNotes/${id}`;
      const urlInProd = `https://mynly.vercel.app/api/notes/getNotes/${id}`;
      const url = nodeEnv === "production" ? urlInProd : urlInDev;
      try {
  ...
  return (
    <>
      {isLoading ? (
        <div className="p-6 pt-24 min-h-screen flex flex-col justify-center items-center lg:ml-64 bg-gradient-to-b from-amber-100 to-white">
          <Loading3 />
        </div>
      ) : (
        <div className="p-6 pt-24 min-h-screen lg:ml-64 bg-gradient-to-b from-amber-100 to-white">
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center px-5 py-3 duration-75 text-base font-medium text-center text-white bg-amber-500 rounded-lg hover:bg-amber-600 focus:ring-4 focus:ring-amber-300 w-full sm:w-52 ${
                    !title
                      ...
                      : "hover:shadow focus:ring-4 focus:ring-amber-300"
                  }`}
                >
                  Update
                </button>
